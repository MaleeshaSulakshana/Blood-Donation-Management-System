...
app_admin_emergency = Blueprint("app_admin_emergency", __name__, url_prefix="/admin/emergency",
...
@app_admin_emergency.route('/needs')
def needs():
    if 'adminId' not in session:
        ...
        return render_template('admin/emergency/view_emergency_needs.html', details=details)


@app_admin_emergency.route('/add')
def add():
    if 'adminId' not in session:
        return redirect(url_for('index'))

    else:
        return render_template('admin/emergency/add_emergency_need.html')


@app_admin_emergency.route('/view')
def view():
    if 'adminId' not in session:
        return redirect(url_for('index'))
        ...
        return render_template('admin/emergency/view_emergency_need_details.html', details=details)


# Route for add campaign
@app_admin_emergency.route('/add_emergency_need', methods=['GET', 'POST'])
...
@app_admin_emergency.route('/remove', methods=['GET', 'POST'])
def remove():
